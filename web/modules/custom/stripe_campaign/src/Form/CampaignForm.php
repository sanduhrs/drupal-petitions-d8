<?php

namespace Drupal\stripe_campaign\Form;

use Drupal\Core\Entity\ContentEntityForm;
use Drupal\Core\Form\FormStateInterface;

/**
 * Form controller for the campaign entity edit forms.
 */
class CampaignForm extends ContentEntityForm {

  /**
   * {@inheritdoc}
   */
  public function save(array $form, FormStateInterface $form_state) {

    $entity = $this->getEntity();
    $result = $entity->save();
    $link = $entity->toLink($this->t('View'))->toRenderable();

    $message_arguments = ['%label' => $this->entity->label()];
    $logger_arguments = $message_arguments + ['link' => render($link)];

    if ($result == SAVED_NEW) {
      $this->messenger()->addStatus($this->t('New campaign %label has been created.', $message_arguments));
      $this->logger('stripe_campaign')->notice('Created new campaign %label', $logger_arguments);
    }
    else {
      $this->messenger()->addStatus($this->t('The campaign %label has been updated.', $message_arguments));
      $this->logger('stripe_campaign')->notice('Updated new campaign %label.', $logger_arguments);
    }

    $form_state->setRedirect('entity.campaign.canonical', ['campaign' => $entity->id()]);
  }

}
